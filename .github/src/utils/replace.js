export function replace(arr, content) {
    ...
    arr.caseInfo.map((item, index) => {
        let val = `<p>（${index + 1}）：<span id="contract_code" style="color: #5488f9;">${item.loanPactNo}</span>元的借款，金额<span id="contract_money" style="color: #5488f9;">${item.loanPactAmount}</span>元；</p>`
        arrs.push(val)
    })
    arrs.pop()
    let str = arrs.join('')
